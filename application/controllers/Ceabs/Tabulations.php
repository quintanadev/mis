...
class Tabulations extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('Ceabs/Tabulations_Model', 'Ceabs');
    }

    public function index() {
        $data['page_title'] = 'Analítico de Resultados';
        $data['page_subtitle'] = 'CEABS';
        $data['views'] = ['pages/operations/ceabs/tabulations'];
    ...
    public function get_result_table() {
        ...
        $retorno = $this->Ceabs->get_result_table($post);
        ...
        if ($retorno) :
            for ($i = $regInicio; $i < $regFim; $i++) :
                $data['data'][] = [
                    'Finalizador' => $retorno[$i]['Finalizador'],
                    'QtdTotalTabulacao' => $retorno[$i]['QtdTotalTabulacao'],
                    '0' => $retorno[$i]['0'],
                    '1' => $retorno[$i]['1'],
                    '2' => $retorno[$i]['2'],
                    '3' => $retorno[$i]['3'],
    ...
    public function get_result_chart_date() {
        $post = $this->input->post();
        $retorno = $this->Ceabs->get_result_chart_date($post);
        echo json_encode($retorno);
    }

    public function get_result_chart_interval() {
        $post = $this->input->post();
        $retorno = $this->Ceabs->get_result_chart_interval($post);
        echo json_encode($retorno);
    }
    ...
    public function export_file_excel() {
        ...
        $retorno = $this->Ceabs->export_file_excel($post);
        ...
        $filename = "Resultado_Ceabs_" . date('YmdHsi') . ".xls";
